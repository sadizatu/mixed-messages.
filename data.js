const love = [
    "I saw that you were perfect, and so I loved you. Then I saw that you were not perfect and I loved you even more",
    "You know you’re in love when you can’t fall asleep because reality is finally better than your dreams",
    "Love is that condition in which the happiness of another person is essential to your own",
    "The best thing to hold onto in life is each other",
    "I need you like a heart needs a beat",
    "I am who I am because of you. You are every reason, every hope, and every dream I’ve ever had",
    "If I had a flower for every time I thought of you… I could walk through my garden forever",
    "Take my hand, take my whole life too. For I can’t help falling in love with you",
    "If you live to be a hundred, I want to live to be a hundred minus one day so I never have to live without you",
    "You’re the closest to heaven, that I’ll ever be",
    "You are the finest, loveliest, tenderest, and most beautiful person I have ever known and even that is an understatement",
    "I will never stop trying. Because when you find the one… you never give up",
    "It’s always better when we’re together",
    "I love you for all that you are, all that you have been and all that you will be",
    "To the world you may be one person, but to one person you are the world",
    "I’ve tried so many times to think of a new way to say it, and it’s still I love you",
    "I love you and that’s the beginning and end of everything",
    "If I know what love is, it is because of you",
    "If you remember me, then I don’t care if everyone else forgets",
    "We loved with a love that was more than love",
]

const grief = [
    "The reality is that you will grieve forever.",
    "Keep your head up. God gives his hardest battles to his strongest soldiers.",
    "You will survive and you will find purpose in the chaos. Moving on doesn't mean letting go.",
    "Grief can be a burden, but also an anchor.You get used to the weight, how it holds you in place.",
    "Grieving doesn 't make you imperfect. It makes you human.",
    "Her absence is like the sky, spread over everything.",
    "And sometimes, against all odds, against all logic, we still hope.",
    "When it is darkest, we can see the stars.",
    "The darker the night, the brighter the stars, the deeper the grief, the closer is God!",
    "When someone you love becomes a memory, that memory becomes a treasure.",
    "This is not goodbye, my darling, this is a thank you. Thank you for coming into my life and giving me joy.",
    "How lucky I am to have something that makes saying goodbye so hard.",
    "The Lord is near to the brokenhearted and saves the crushed in spirit.",
    "I can do all this through him who gives me strength.",
    "Tears shed for another person are not a sign of weakness.They are a sign of a pure heart.",
    "Life is full of grief, to exactly the degree we allow ourselves to love other people.",
    "To live in hearts we leave behind is not to die.",
    "Give all your worries to Him, because He cares for you.",
    "He gives strength to the weary and increases the power of the weak.",
    "Despite the forecast, live like it 's Spring."
]

perseverance = [
    "Success is no accident.",
    "Perseverance is not a long race;it is many short races one after the other.",
    " Life is not easy for any of us.",
    "Great works are performed not by strength but by perseverance.",
    " Patience and perseverance have a magical effect before which difficulties disappear and obstacles vanish",
    " Perseverance, secret of all triumphs.",
    "Life is the flower for which love is the honey.",
    "Never giving up",
    " Not being limited by time constraints",
    "Staying rooted in the confidence that success is inevitable",
    "Perseverance is not a long race;it is many short races one after the other.",
    "The difference between the difficult and the impossible is that the impossible takes a little longer time.",
    "Rivers know this: there is no hurry.We shall get there some day.",
    "Some lean back.But those who lean forward are poised to cross the finish - line, first!",
    "It’ s not that I’ m so smart, it’ s just that I stay with problems longer.",
    "Let me tell you the secret that has led to my goal.My strength lies solely in my tenacity.",
    "The greatest oak was once a little nut who held its ground.",
    "Age wrinkles the body.Quitting wrinkles the soul.",
    "How long should you try ? UNTIL!",
    " It always seems impossible until it’ s done."


]

const sadness = [

    "Sadness is also a kind of defense.",
    " Our sweetest songs are those that tell of saddest thought.",
    "Sadness flies away on the wings of time.",
    "Sad hurts but its a healthy feeling.",
    "Proud people breed sad sorrows for themselves.",
    "Melancholy is the happiness of being sad.",
    "You 're going to make it. It.be hard but you 're going to make it.",
    "Nothing can cure the soul but the senses as just as nothing can cure the senses but the soul.",
    "Trust is like an eraser, it gets smaller and smaller after every mistake.",
    " In deep sadness there is no place for sentimentality.",
    "Our sweetest songs are those that tell of saddest thought.",
    "It hurts when you have someone in your heart but you can 't have them in your arms. ",
    "You fall in love with the most unexpected person at the most unexpected time",
    " Don 't tell me I've changed, when in reality, I just stopped dealing with you.",
    "You will never find the right person if you never let go of the wrong one.",
    "If two people are meant to be together, they will eventually find their way back into each other 's arms... no matter what.",
    "Keep smiling...and one day life will get tired of upsetting you.",
    "It sucks, doesn 't it? Feeling like you are not good enough.",
    "I try not to miss you, Itry to let you go, but in the end, you 're always on my mind.",
    "Forget all the reasons why it won 't work, and believe the one reason why it will.",

]


const happiness = [

    " Happiness is a direction,not a place.",
    " Happiness depends upon ourselves.",
    "Happiness is a form of courage.",
    "Happiness is not a goal;it is a by - product.",
    "Happiness is only real when shared.",
    "No medicine cures what happiness cannot.",
    " Happiness lies in the joy of achievement and the thrill of creative effort",
    "Happiness is the interval between periods of unhappiness",
    "Happiness is having a large, loving, caring, close - knit family in another city",
    "The secret of happiness is to find a congenial monotony",
    "Now and then it’ s good to pause in our pursuit of happiness and just be happy.",
    "It is not how much we have, but how much we enjoy, that makes happiness",
    "The secret of happiness is freedom, the secret of freedom is courage",
    "Happiness is a by - product of an effort to make someone else happy.",
    " Nobody really cares if you’ re miserable, so you might as well be happy ",
    "Happiness grows at our own firesides, and is not to be picked in strangers’ gardens",
    "Happiness is not something ready made.It comes from your own actions.",
    "The searchfor happiness is one of the chief sources of unhappiness.",
    "The only way to find true happiness is to risk being completely cut open.",
    "All happiness depends on courage and work",

]

const forgiveness = [

    " We are not perfect, forgive others as you would want to be forgiven.",
    " Forgiveness eliminates the wounds of the past from the mind and the heart.",
    "Real forgiveness is a lifelong commitment.",
    "Forgiveness is for you - not the other person ",
    "Don't ever underestimate the power of forgiveness.",
    "Forgiving other people who have wronged us or hurt us or embarrassed us is not easy.",
    "God offers us the very best, His perfect plan for our life which includes His Son, love, forgiveness, and peace.",
    "Don't get angry quickly - be kind, be forgiving, be patient.",
    "There are always people who can't forgive an able man for differing from them.",
    " If we are looking for something to be thankful for, we can be thankful for God 's forgiveness.",
    "We should never be surprised when God keeps His word to forgive those who confess their sins.",
    "He can salve the bitterness, slake the fear, give you a heart of compassion and forgiveness",
    "Jesus died so we humans can find mercy and forgiveness",
    "Life is so short. Grudges are a waste of time.",
    "One always begins to forgive a place as soon as it's left behind.",
    "God 's forgiveness and grace is bigger than anything wrong you've ever done or will do .",
    "There are always people who can't forgive an able man for differing from them.",
    "Resolve today to examine your life. Bring any sin before the throne of grace and experience his mercy and forgiveness today.",
    " Because I make mistakes, I know others do too.I 've found when I give forgiveness, it",
    "is me - not the other person - who benefits."

]

const hardwork = [

    "Concentrate all your thoughts upon the work in hand.",
    "Either you run the day or the day runs you.",
    "I 'm a greater believer in luck, and I find the harder I work the more I have of it.",
    "When we strive to become better than we are, everything around us becomes better too",
    " Develop success from failures.Discouragement and failure are two of the surest stepping stones to success .",
    "Nothing in the world can take the place of Persistence.",
    "I never dreamed about success.I workedfor it.",
    "Success is getting what you want, happiness is wanting what you get.",
    "When you change your thoughts, remember to also change your world.",
    "If you are working on something that you really care about, you don’ t have to be pushed.The vision pulls you.",
    "To know how much there is to know is the beginning of learning to live.",
    "It is only when we take chances, when our lives improve ",
    " I’ m a greater believer in luck, and I find the harder I work the more I have of it.",
    "The road to success and the road to failure are almost exactly the same.",
    "Setting goals is the first step in turning the invisible into the visible.",
    "It is better to fail in originality than to succeed in imitation.",
    "When we strive to become better than we are, everything around us becomes better too.",
    "Success is not final; failure is not fatal: It is the courage to continue that counts.",
    "Opportunity is missed by most people because it is dressed in overalls and looks like work", "Success usually comes to those who are too busy looking for it."
]




console.log(love.length);
console.log(grief.length);
console.log(perseverance.length);
console.log(sadness.length);
console.log(hapiness.length);
console.log(forgiveness.length);
console.log(hardwork.length);